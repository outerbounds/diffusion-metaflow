...
def get_outputs_for_text_to_video_foreach(
    ...
    ret_data = []
    for idx, run in enumerate(get_runs("TextToVideoForeach", **kwags)):
        if max_runs is not None and idx >= max_runs:
            break
        if not run.successful:
            continue
        ret_data.append((run, run.data.style_outputs))
    return ret_data
...
def stitch_videos(video_paths, output_path, fade_duration=1, film_fps=24, prompts=[]):
    ...
    if len(prompts) > 0:
        ...
        assert len(prompts) == len(
            video_paths
        ), "If you pass prompts arg to stitch_videos, it must be same length as video_paths."
        clips = []
        for path, prompt in zip(video_paths, prompts):
            ...
            text_col = text.on_color(
                size=(video_clip.w // 2 + text.w, text.h + 10),
                color=(0, 0, 0),
                pos=(6, "center"),
                col_opacity=0.4,
            )
        ...
        clips = [
            add_fade_animation(VideoFileClip(path), fade_duration)
...
def download_foreach_run_videos(
    ...
    with S3(s3root=f"{DATATOOLS_S3ROOT}{flow_name}/{run_id}") as s3:
        ...
        mp4_paths = dedupe_ls([p.url for p in all_paths if p.url.endswith(".mp4")])
...
def make_movie_from_foreach_run(
    ...
    prompt_to_path = download_foreach_run_videos(
        flow_name, run_id, step_name, save_folder
    )
    video_paths = list(prompt_to_path.values())
    prompts = list(prompt_to_path.keys())
    if (
        len(video_paths) > 1
        and max_video_in_film <= len(video_paths)
        and max_video_in_film is not None
    ):
