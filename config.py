...
class ModelStoreConfig:
    pretrained_model_path: str = MODEL_PATH # Path to the downloaded model on the local machine.
    force_upload: bool = False # Force upload the model from the local machine
    s3_prefix : str = MODELS_BASE_S3_PATH # prefix of the path where models are stored in S3.
...
class ImageStylePromptDiffusionConfig:
    model_config: ImageModelConfig = field(default_factory=ImageModelConfig)
    inference_config: ImageInferenceConfig = field(default_factory=ImageInferenceConfig)
    style_prompt_config : ImageStylePromptConfig = field(default_factory=ImageStylePromptConfig)

@dataclass
class ImageToVideoDiffusionConfig:
    model_config: VideoModelConfig = field(default_factory=VideoModelConfig)
    inference_config: VideoGenerationConfig = field(default_factory=VideoGenerationConfig)

@dataclass
class TextToVideoDiffusionConfig:
    video : ImageToVideoDiffusionConfig = field(default_factory=ImageToVideoDiffusionConfig)
    image : TextToImageDiffusionConfig = field(default_factory=TextToImageDiffusionConfig)
...
if __name__ == "__main__":
    import sys
    assert len(sys.argv) == 3, "usage : `python config.py <type> example.yaml`. Allowed <type> : `text`, `style`, `video`"
    if sys.argv[1] == "text":
